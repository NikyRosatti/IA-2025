...
GENOME_LENGTH = 3 # Longitud de cada individuo (número de bits)
COLOR = [122, 200, 4]
# Genotipo
...
def fitness(individual):
    res = sqrt((individual[0] - COLOR[0])**2+(individual[1] - COLOR[1])**2+(individual[1] - COLOR[1])**2)
    return res
# Evolucion: ...
...
def mutate(individual):
    for i in range(GENOME_LENGTH):
        if random.random() < MUTPB:
            individual[i] = random.randint(0,255) # Flip bit
    return individual
# Cruzamiento
def crossover(p1,p2):
