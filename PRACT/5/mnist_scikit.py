...
def train_baseline_model(X_train, y_train, X_test, y_test):
    ...
    model = MLPClassifier(
        ...
        learning_rate_init=0.1,         # Learning rate conservador para asegurar convergencia
        batch_size=10,                  # mini-batch de 10 (como en el ej 1)
        max_iter=50,                    # Más épocas para mejor convergencia
        ...
        n_iter_no_change=20             # Paciencia antes de parar
    )
    
    print("\nNota: Si el modelo converge a predecir solo 1-2 clases,")
    print("      intenta reducir learning_rate_init a 0.05 o 0.01")
    print("      o aumentar max_iter a 100+\n")
...
def train_with_different_hyperparameters(X_train, y_train, X_test, y_test):
    ...
    model_high_lr = MLPClassifier(
        ...
        activation='logistic',
        solver='sgd',
        learning_rate_init=0.3,  # Learning rate moderado
        momentum=0.0,
        nesterovs_momentum=False,
        batch_size=10,
        max_iter=50,
    ...
    results['Learning rate moderado (η=0.3)'] = acc_high_lr
